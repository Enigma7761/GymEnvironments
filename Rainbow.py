...
class Rainbow:
    def __init__(self, Model, minibatch_size=16, replay_memory_size=200000, gamma=0.99, learning_rate=5e-4,
                 tau=1e-4, param_noise=0.1, max_distance=0.2, alpha=0.5, beta=0.5, burn_in=40, seq_len=80):
    ...
    def choose_action(self, state):
        ...
        action = torch.argmax(self.value(state), dim=2).detach().cpu().numpy()
        self.value.train()
        return action
